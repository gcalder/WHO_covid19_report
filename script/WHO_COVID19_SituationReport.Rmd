...
Caveat: Colour scales are non-linear and are re-calibrated every week.
...
As of `r format(today, "%d/%m/%Y")`, `r length(AF$Collection.date)` SARS-CoV-2 genomes collected and sequenced in Africa have been submitted to GISAID. Each genome's country of origin and collection date is summarised below:
...
```{r sequencingUpdate, fig.align = 'center', out.width= "70%"}
...
for( i in country_list){
 ...
 current_country_increase <-  WHO_cases_and_deaths_7_day_increase %>%
  ...
  cases_per_10k_plot <- ggplot() +
    ...
    scale_y_log10(breaks = c(0.1,0.01,0.001,0.0001),labels = c("0.1","0.01","0.001","1e-4")) +
    scale_x_date(date_labels = "%b %d",breaks=c(today-56,today-42,today-28,today-14,today)) +
